...
class LoadConfigTests(unittest.TestCase):
    ...
    def test_display_power_on_effect_default_is_true(self) -> None:
        with _Tempdir() as td:
            self._write(td / "companion_app.config.json", _with_password({}))
            cfg = load_and_resolve_config(None)
        self.assertTrue(cfg.display_power_on_effect)

    def test_display_power_on_effect_false_honored(self) -> None:
        with _Tempdir() as td:
            p = self._write_pw(td, {"display": {"powerOnEffect": False}})
            cfg = load_and_resolve_config(str(p))
        self.assertFalse(cfg.display_power_on_effect)

    def test_display_power_on_effect_must_be_bool(self) -> None:
        with _Tempdir() as td:
            p = self._write_pw(td, {"display": {"powerOnEffect": "yes"}})
            with self.assertRaises(ConfigError) as ctx:
                load_and_resolve_config(str(p))
            self.assertIn("display.powerOnEffect", str(ctx.exception))
